...
from fairseq.modules import LayerNorm, PositionalEmbedding, TransformerDecoderLayer, Fp32GroupNorm, Fp32LayerNorm, TransposeLast
...
@register_model("wav2vec_cip_amsoftmax")
...
#   #     add_common_args(parser)
    ...
    def __init__(self, args, tgt_dict=None, mode: str = "default",conv_bias: bool = False, dropout: float = 0.0):
        ...
        if self.args.produce_weights_type == 'conv':
            def block(
                ...
                def make_conv():
                    conv = nn.Conv1d(n_in, n_out, conv_cif_width, stride=1, bias=conv_bias, padding=2)
            ...
            in_d = 768
            self.conv_cif_widths_list = self.args.conv_cif_width_string.split(',')
            self.conv_layers = nn.ModuleList()
            for i in range(self.args.conv_cif_num_layers):

                dim = self.args.conv_cif_num_filters
        ...
        elif self.args.produce_weights_type == 'dense':
            self.cif_dense = Linear(768, self.dense_cif_units)

        self.final_dense = Linear(self.args.dense_cif_units, 1)
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        if self.args.produce_weights_type == 'conv':
            a = x.transpose(1,2)
            assert len(self.conv_cif_widths_list) == self.args.conv_cif_num_layers
        ...
        elif self.args.produce_weights_type == "none":
            pass
        else:
            raise ValueError("No such weight-producing type!")


        a = self.final_dense(a)
        ...
        ax = x.mul(a.repeat(1,1,x.size(2)))

        #print('ax0', ax)
        #print('a0', a)
        sum_a = torch.ones_like(a)
        sum_ax = torch.ones_like(ax)
        for t in range(1, ax.size(1)):
            sum_a[:,t,:] = torch.sum(a[:,:t+1,:], dim=1)
            sum_ax[:,t,:] = torch.sum(ax[:,:t+1,:], dim=1)
            #a[:,t,:] = a[:,t-1,:] + a[:,t,:]


        pred_flag = torch.ones((sum_ax.size(0),1)) < 0
        pred_flag = pred_flag.cuda()

        pre_pred_flag = pred_flag
        pre_sum_ax_t = torch.zeros((sum_ax.size(0), sum_ax.size(2))).cuda()
        for t in range(sum_ax.size(1)):
            sum_ax_t = sum_ax[:,t,:]
            sum_a_t = sum_a[:,t,:]


            sum_ax_t = sum_ax_t * (1-pre_pred_flag.float()) + pre_sum_ax_t * pre_pred_flag.float()
            pred_flag = sum_a_t > 10

            #pred_flag = pred_flag.unsqueeze(1)
            #print(pred_flag.size())
            pred_flag = pred_flag | pre_pred_flag | padding_mask[:,t].unsqueeze(1)

            pre_pred_flag = pred_flag
            pre_sum_ax_t = sum_ax_t

            if pred_flag.all():
                break

        xx = sum_ax_t
        xx_norm = torch.norm(xx, p=2, dim=1, keepdim=True).clamp(min=1e-12)
        xx_norm = torch.div(xx, xx_norm)
        w_norm = torch.norm(self.W, p=2, dim=0, keepdim=True).clamp(min=1e-12)
        w_norm = torch.div(self.W, w_norm)
        costh = torch.mm(xx_norm, w_norm.float())
...
@register_model_architecture("wav2vec_cip_amsoftmax", "wav2vec_cip_amsoftmax")
